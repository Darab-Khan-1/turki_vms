    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12">
                <h3 class="content-header-title"><?= lang('Text.updateGeofence') ?></h3>
            </div>
            <div class="content-header-right breadcrumbs-right col-md-6 col-12 d-none d-md-block">
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item pl-0"><a href="<?= base_url('panel/geofence') ?>"><?= lang('Text.geofence') ?></a></li>
                        <li class="breadcrumb-item pl-0 active"><?= lang('Text.updateGeofence') ?></li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <section id="geofenceCard" class="card">
                <div class="card-content collapse show">
                    <div class="card-header">
                        <h4 class="card-title" id="updateUserCardTitle"><?=lang('Text.updateGeofence')?></h4>
                    ...
                    <div class="card-body py-0">
                        <form class="form updateGeofenceForm" id="updateGeofenceForm" name="updateGeofenceForm"
                              novalidate>
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-3">
                                        <h4 class="form-section" style="margin-bottom: 12px;"><?=lang('Text.basicInformation')?></h4>
                                        <div class="form-group">
                                            <div class="controls">
                                                <label for="name"><?=lang('Text.name')?></label>
                                                <input type="text" id="name" class="form-control" placeholder="<?=lang('Text.name')?>"
                                                       name="name" required
                            ...
                            <div class="form-actions text-right">
                                ...
                                <button type="button" class="btn btn-info" id="saveUpdateGeofenceButton">
                                    <?=lang('Text.save')?>
                                </button>
                            </div>
                        </form>
...
<script>
    $(document).ready(function () {
        ...
        let geofence = JSON.parse('<?=$geofenceUpdate;?>');
        ...
        $('#updateGeofenceForm #id').val(geofence.id);
        $('#updateGeofenceForm #name').val(geofence.name);
        $('#updateGeofenceForm #wktData').val(geofence.area);
        $('#updateGeofenceForm #description').val(geofence.description);
        $('#updateGeofenceForm #calendarData').val(geofence.calendarId).trigger('change');
        //$('#updateGeofenceForm #color').val(geofence.attributes.color);

        $('#updateGeofenceForm #color').val(geofence.attributes.color);
        $('#updateGeofenceForm #color').minicolors({
        ...
        if (speedUnit === 'kmh') {
            let speedLimit = $.number(parseFloat(geofence.attributes.speedLimit) * 1.852, '0', '', '');
            $('#updateGeofenceForm #speedLimit').val(speedLimit);
            $('#updateGeofenceForm #speedUnit').text('Km/h');
            //$('#deviceInfoSpeed').val(device["position"]["speed"] + ' Kn');
        } else {
            $('#updateGeofenceForm #speedLimit').val(geofence.attributes.speedLimit);
            $('#updateGeofenceForm #speedUnit').text('Kn');
        }
        if (distanceUnit === 'km') {
            let polylineDistance = $.number(parseFloat(geofence.attributes.polylineDistance) / 1000, '0', '', '');
            $('#updateGeofenceForm #polylineDistance').val(polylineDistance);
            $('#updateGeofenceForm #distanceUnit').text('Km');
            //$('#deviceInfoSpeed').val(device["position"]["speed"] + ' Kn');
        } else {
            $('#updateGeofenceForm #polylineDistance').val(geofence.attributes.polylineDistance);
            $('#updateGeofenceForm #distanceUnit').text('m');
        }
        ...
        let map = L.map('mapDiv').setView([0, 0], 13);
        ...
        if (geofence.area.includes("CIRCLE")) {
            ...
            let latlng = new L.LatLng(latlngArr[0], latlngArr[1]);
            map.setView(latlng, 13);
            ...
            let circle = L.circle(latLng, style).addTo(drawnItems);
            ...
            if (geojson.type === "Polygon") {
                let polygon = L.polygon(geojson.coordinates[0], style).addTo(drawnItems);
                map.fitBounds(polygon.getBounds());
            }
            if (geojson.type === "LineString") {
                let polyline = L.polyline(geojson.coordinates, style).addTo(drawnItems);
        ...
        let updateGeofenceForm = $('#updateGeofenceForm');
        let saveUpdateGeofenceButton = $('#saveUpdateGeofenceButton');

        let userValidation = FormValidation.formValidation(updateGeofenceForm[0], {
            fields: {
                name: {
                    validators: {
                        notEmpty: {
        ...
        updateGeofenceForm.on('keypress', function (e) {
            if (e.which === 13) {
                saveUpdateGeofenceButton.trigger('click');
            }
        });
        let headers = {
            'Authorization': 'Basic ' + '<?=$btoa?>'
        };
        saveUpdateGeofenceButton.on('click', function (e) {
            e.preventDefault();
            userValidation.validate().then(function (status) {
                if (status === 'Valid') {
                    ...
                    $.axiosPost("<?=base_url('rest/setting/update-geofence')?>", headers, updateGeofenceForm.serializeToJSON({}), geofenceCard, callback);
                }
            });
        });
    });
